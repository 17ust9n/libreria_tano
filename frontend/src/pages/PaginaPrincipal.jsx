...
function PaginaPrincipal() {
  ...
  const fetchLibros = async () => {
    try {
      ...
      const inicial = {};
      data.forEach(libro => { inicial[String(libro.id)] = 1; });
  ...
  const eliminarLibro = async (id) => {
    try {
      ...
      fetchLibros(); // recarga los libros después de eliminar
      setLibroSeleccionado(null); // cierra el modal
    } catch (error) {
      console.error(error);
      alert("No se pudo eliminar el libro: " + error.message);
    }
  };
              ...
              return (
                <div key={libro.id} style={{
                  ...
                  <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 10, marginTop: 10 }}>
                    <span><b>Stock:</b></span>
                    <button onClick={() => incrementar(libro.id)} style={{ backgroundColor: colores.boton, border: "none", borderRadius: 5, padding: "5px 8px", cursor: "pointer" }}>⬆️</button>
...
{libroSeleccionado && (
  <div style={{
    ...
    <div style={{
      ...
      <button
      onClick={() => eliminarLibro(libroSeleccionado.id)}
      style={{
    backgroundColor: "red",
    border: "none",
    borderRadius: 5,
